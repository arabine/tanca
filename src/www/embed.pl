...
foreach my $i (@Files) {
  ...
  my $FullPath = $RootDir.$i;
  
  open FD, '<:raw', $FullPath or die "Cannot open $FullPath: $!\n";
  printf("static const unsigned char v%d[] = {", $counter);
  my $byte;
  my $j = 0;
...
foreach my $i (@Files) {
    ...
    my ($name,$path,$suffix) = fileparse("$i",qr"\..[^.]*$");
    my $mime = GetMime($suffix);
    
    print "  {\"/$i\", \"$mime\", v$counter, sizeof(v$counter) - 1},\n";
    $counter = $counter + 1;
}
